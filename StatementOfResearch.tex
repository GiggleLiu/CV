...
%I want to discover the fundamental law of physics that governing this physical system without translational invariance in both time and space.
%For example, I will try to see how energy consumption is related to the target problem and computing speed from the quantum thermal dynamics' perspective.
%My second wildest dream is to propose a general proposed automated theorem proving framework, which I think is a technology that might change the world significantly and make a lot theorists lost their jobs.
%This dream is too ambitious to make a concrete plan, in order to this research statement more trust worthy, 
...
%This is because in traditional reversible computing, most proposed frameworks are the von-Neuman architecture that requires a RAM.
...
%because current CMOS based computing devices has so much room to improve and improving the energy efficiency would bring significant changes to our society.
%It is estimated that current CMOS based computation is $10^6$ times less energy efficient comparing to the the brain of a honeybee.
%I believe if we can overcome this energy barrier, machine learning may boost for the enormous computational power and strong artificial intellectual may emerge.
...
%Limited resources and the perpetuation of human race require us not only to explore the universe to find more resources, but also to make better use of existing resources on the earth, e.g. the energy resource.
...
%The algorithms in the hub can be specified by a dictionary of input arguments and a dictionary of outputs.
...
%Developers can update their application to this web server together with a complexity model.
